...
scribblelive.prototype.get_audience = function(next)
{
	...
	{
		superagent.agent()
			.post("https://client.scribblelive.com/api/rest/sherlock/filter/client/" + this.data.client_id + "/audience/segments/0/?Format=json&SortOrder=DESC&PageLimit=100&Token=" + this.token + "&Auth=" + user_auth)
			.send({
				FilteredOptions: [			{"Type":"Name"},{"Type":"Email"},{"Type":"Type"},{"Type":"DateLastSeen"},{"Type":"TotalEngagedTime"},{"Type":"PiecesOfContentViewed"}],
				Segment:{"match":"ALL","filters":[{"option":{"type":"Email"},"comparator":"IS"}]},
				SortBy:{"Type":"DateLastSeen"},
				StartDate:1421798400000,
				EndDate:1422403199000
			})
			.end(function(err, res)
			{
				if(err)
				...
				if(res.body.Result && res.body.Result.length > 0 && res.body.Result.Audience )
				res.body.Result.forEach(function(result)
				{
					profiles.push(result.Audience);
				});
